...
module.exports.playgroundTests = describe('Playground', function() {
	const globalVar = 'global';

	describe('mocha.js', function() {
		...
		describe('arrow functions', function() {
			...
			it('this does not exist bc arrow function', function() {
				expect(this).toStrictEqual({});
			});

			it('can access global variable', function() {
				expect(globalVar).toBe('global');
			});

			it(
				'can pass named function',
				(arrowNamed = () => {
					expect(globalVar).toBe('global');
				}),
			);

			it('cannot grab hookVar from context', function() {
		...
		describe('timeouts', function() {
			...
			describe('slow', function() {
				...
				describe('should have yellow times', function() {
					it('calling slow() before suite applies to all tests in suite test 1', function(done) {
						setTimeout(done, 2000);
					});

					it('calling slow() before suite applies to all tests in suite test 2', function(done) {
						setTimeout(done, 2000);
					});
				});
			});
