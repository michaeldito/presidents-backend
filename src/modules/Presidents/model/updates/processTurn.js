...
module.exports = async function(turn) {
  ...
  console.log(`[Presidents@processTurn()] turn: ${JSON.stringify(turn, null, 2)}`);
  ...
  try {
    if (turn.endedRound) {
      console.log('[Presidents@processTurn()] Player played a round ending turn!')
  ...
  console.log(`[Presidents@processTurn()] didPlayerPlayCards: ${didPlayerPlayCards}`)
  ...
  if (didPlayerPlayCards) {
    ...
    console.log(`[Presidents@processTurn()] cardsToKeep: ${currentPlayer.hand.map(card => card.shortHand)}`)
  }
  ...
  console.log(`[Presidents@processTurn()] isPlayerOutOfCards: ${isPlayerOutOfCards}`)
  if (isPlayerOutOfCards) {
    const finishedPlayers = this.players.filter(player => player.nextGameRank);
    console.log(`[Presidents@processTurn()] finishedPlayers: ${finishedPlayers}`)
    const nextGameRankValue = finishedPlayers.length + 1;
    console.log(`[Presidents@processTurn()] nextGameRankValue: ${nextGameRankValue}`);
