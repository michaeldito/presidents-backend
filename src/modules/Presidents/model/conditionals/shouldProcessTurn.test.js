	...
	describe('#shouldProcessTurn()', function() {
		describe('true', function() {
			it('current hand is better', async function() {
				...
				try {
					shouldProcessTurn = await Presidents.shouldProcessTurn(
						currentPlayer,
						handToBeat,
						turn,
					);
		...
		describe('false', function() {
			...
			it('cards are not better', async function() {
				...
				try {
					...
					expect(err.message).toBe(
						`Unable to process turn. The selected cards are not better. The rank of the selected cards does not beat the previous turns.`,
					);
				}
			});
		});
